#!/usr/bin/env node
// Build perf-corpus/ — 7500-file synthetic corpus for JWST renderer perf tests.
//
// 10 star types × 750 variants = 7500 files. Filenames carry the star_type
// prefix so seed-perf.ts assigns types deterministically (same convention as
// build-audit-corpus.mjs, just larger and with 4-digit indices).
//
// Usage: node scripts/build-perf-corpus.mjs
// Then:  npx tsx scripts/seed-perf.ts
// Or:    bash scripts/perf-jwst-7500.sh   (one-shot)
...
const OUT = join(ROOT, 'perf-corpus')
...
## Filler
This file exists for the JWST renderer perf test bed. The corpus contains
${TYPES.length} star types × ${PER_TYPE} variants = ${TYPES.length * PER_TYPE} files,
sized to reproduce the typed sprite cache thrash that triggered the bake-budget fix.
`
  ...
  console.log(`perf-corpus built: ${count} files at ${OUT}`)
  console.log(`Next: npx tsx scripts/seed-perf.ts`)
}
