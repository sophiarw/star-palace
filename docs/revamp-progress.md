...
## Blind-review browsing corrections and hybrid labels

Stars now have at least a 24 CSS-pixel hit radius at every zoom, including unhydrated overview markers. The nearest star takes priority over broad heading targets. Selecting an overview marker opens its preview without an unexpected camera flight; explicit related-file navigation still focuses its destination.

Library region buttons isolate actual member stars, clouds and folder lines without moving their persistent positions. Re-selecting a region fits it again rather than restoring a stale region camera. The Udemy report was reproduced as a 151-file region spread across much of the existing preview; an honest fit still spans those coordinates, but unrelated stars no longer fill it. Global Auris retrieval already returned nine matches; geographic search scoping caused the surprising omission. Search now spans regions unless the user explicitly selects a narrower Search area, retaining other filters.

The follow-up extension request replaces broad categories in the visible filter with actual suffixes and counts, including no-extension files. Summary, hydration, list, lexical and related retrieval share the filter. Switching filters also replaces summary markers when aggregate counts happen to match. The object guide has wider gaps/padding, larger description text, a single column on narrow screens, and shorter nebula copy.

At the user's request, always-persistent headings are superseded by the hybrid approach in [sky-labels.md](sky-labels.md): nested spatial groups named from folder labels, with continuous zoom fades and fixed spacing priorities. Names give way to filenames up close. Hover and hydration do not rebuild label groups. File positions and user metadata remain authoritative and unchanged.

Validation: 477 unit/integration tests passed (3 historical skips); the full 49-check app browser suite passed, including selection before hydration, region isolation/revisit, global versus explicit geographic search, equal-count extension switching, label levels, hover pixel stability, zoom continuity, all 17 tutorials, and existing solar-system/editor workflows. Typecheck, lint, renderer and daemon builds passed. Tutorial screenshots were regenerated only from the guarded fictional library. The first unit run lacked sandbox permission for temporary HTTP listeners and was rerun successfully with that permission. No new foreground performance claim or schema migration.
...
## Private website feedback activation

The recipient approved FormSubmit, confirmed inbox activation, and supplied its public opaque form ID. The website configuration now enables email feedback; the recipient address remains absent from public source. The earlier approval block is resolved. Original command-line activation requests timed out; Chrome reached the provider and returned its activation acknowledgment.

Validation with the ID enabled: typecheck, lint, site build, and ten website browser checks passed; the superseded GitHub-flow check is skipped when email is configured. Built HTML contains the opaque endpoint and no Gmail recipient address. A live check from localhost returned FormSubmit's activation-required response, while the same approved ID submitted from `https://starpalace.ai` returned HTTP 200 with `success: true`. The user also confirmed receipt of setup mail. This records provider acceptance from the canonical site; local browser tests continue to intercept submissions.

## Website observing-atlas redesign

The user requested a website makeover using the approved Clouds & landmarks language and the reference article's identity/sketch/asset workflow. The design sketch is in `docs/design/website-observing-atlas.md`. During implementation, the user explicitly replaced the earlier copy constraints: the headline is now “A constellation of files.” Other text uses labels, nouns, captions, and necessary instructions. The old stargazing/SharePoint paragraph is retired.

The page now opens with an asymmetric title and explorable sky, followed by a stellar key, screenshot tutorial, installation commands, and email feedback. Removed the simulated window/sidebar chrome and accumulated CSS overrides. Canonical app sprites and byte-size rules remain shared; faint folder lines and authored fictional cloud groups retain separate meanings. Dust and sprites render only on resize or interaction. The background calligraphy stays at 10% opacity. The approved vector logo keeps its brush silhouette with star cores, rays, halos, and clipped points; the favicon and share card match.

Tutorial screenshots reuse only the guarded fictional library. The first screenshot is a public asset for no-JavaScript HTML; the remaining images are lazy step-specific Vite assets. All four screenshots have full-resolution links. The email endpoint and recipient privacy remain intact, with revised field labels and intercepted provider checks.

Validation: typecheck, lint, 477 repository tests passed (3 historical skips), renderer/site builds, and all ten applicable website browser checks passed (the configured-email form supersedes the skipped GitHub draft test). The initial repository test run encountered the documented native teardown crash; its clean rerun passed. The added no-JavaScript image check caught a development asset-path issue, which was fixed and rerun. Inspected desktop/mobile layouts, and the vector logo at 32/48/128/240 CSS pixels. Site JavaScript is 6.80 kB gzipped, CSS 3.95 kB gzipped; no added runtime dependencies or continuous animation. These bundle sizes are not a foreground performance measurement.

Rollback baseline: `6817b1b`. Website-only release; no library or database changes. Development remains in the atlas worktree.
