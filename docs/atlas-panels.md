...
Search opens results as before. **Close search results panel** hides the result panel while preserving the query and map highlights. **Results** brings it back. Changing the query reveals the results again; **Clear search** removes the query and its highlights. Search spans regions by default. **Search area** can explicitly narrow it to the visited region; an empty geographic search offers **Search all regions**, preserving source, extension, tag, and collection filters.
...
Region buttons now isolate their member stars, labels, haze and folder lines at unchanged world positions. Clicking a region again refits its members instead of restoring an arbitrary zoom. Spatial sky labels instead fit their nearby stars; they do not claim folder membership. See [sky labels](sky-labels.md).
