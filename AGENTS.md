...
- `src/renderer/src/atlas/regionLabels.ts` derives nested spatial labels named from dominant folder labels. Broad groups cross-fade into clusters, which disappear before close reading. Fixed anchors, spacing priorities and zoom-only opacity prevent hover/hydration flicker; see [docs/sky-labels.md](docs/sky-labels.md). File captions keep their separate collision/fade budget.
