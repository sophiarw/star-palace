import { Tutorials, TutorialLink } from './Tutorials'
import { HistorySources } from './TextHistory'
import { UpdatePanel, useUpdateReconnect } from './UpdatePanel'
...
export default function AtlasApp() {
  ...
  useEffect(() => { const open = (event: Event) => setTutorial((event as CustomEvent<string>).detail); window.addEventListener('starpalace-tutorial', open); return () => window.removeEventListener('starpalace-tutorial', open) }, [])
  const [lens, setLens] = useState<AtlasLens>('visible')
  const [fullscreen, setFullscreen] = useState(false), [fullscreenSearch, setFullscreenSearch] = useState(false)
  ...
  const [expanded, setExpanded] = useState(false), [dialog, setDialog] = useState<Dialog>(null), [sidebar, setSidebar] = useState(() => readStored('sidebar', false))
  const [previewVisible, setPreviewVisible] = useState(false), [focusPreview, setFocusPreview] = useState(false)
  const previewToggle = useRef<HTMLButtonElement>(null), libraryToggle = useRef<HTMLButtonElement>(null), resultsToggle = useRef<HTMLButtonElement>(null)
  ...
  const [constellations, setConstellations] = useState<'all' | 'focus' | 'off'>(() => readStored('constellations', 'all'))
  ...
  useEffect(() => { writeStored('sidebar', sidebar) }, [sidebar])
  useEffect(() => { writeStored('constellations', constellations) }, [constellations])
  ...
  const selectedModified = summary.markers?.find(f => f.id === selectedId)?.modifiedAt ?? files.find(f => f.id === selectedId)?.modifiedAt ?? visibleFiles.find(f => f.id === selectedId)?.modifiedAt
  useEffect(() => {
    ...
    return () => abort.abort()
  }, [selectedId, refresh, selectedModified])
  useEffect(() => {
  ...
  const closeLibrary = useCallback(() => { setSidebar(false); requestAnimationFrame(() => libraryToggle.current?.focus({ preventScroll: true })) }, [])
  const hideResults = useCallback(() => { setResultsVisible(false); requestAnimationFrame(() => resultsToggle.current?.focus({ preventScroll: true })) }, [])
  const showPreview = useCallback(() => { setPreviewVisible(true); setFocusPreview(true) }, [])
  const hidePreview = useCallback(() => {
    setPreviewVisible(false); setExpanded(false); setFocusPreview(false)
    requestAnimationFrame(() => previewToggle.current?.focus({ preventScroll: true }))
  }, [])
  ...
  const toggleFullscreen = () => {
    if (fullscreen) {
      const previous = fullscreenReturn.current
      setFullscreen(false); setFullscreenSearch(false)
      if (previous) { setView(previous.view); setExpanded(previous.expanded); setPreviewVisible(previous.preview); requestAnimationFrame(() => previous.focus?.isConnected && previous.focus.focus({ preventScroll: true })) }
    } else {
      fullscreenReturn.current = { view, expanded, preview: previewVisible, focus: document.activeElement as HTMLElement | null }
      setView('map'); setExpanded(false); setFullscreen(true); setFullscreenSearch(false)
      requestAnimationFrame(() => { const stage = document.querySelector<HTMLElement>('.atlas-stage'); if (stage) { stage.tabIndex = -1; stage.focus({ preventScroll: true }) } })
    }
  }
  const vim = useVimBrowsing<Place>({
    tutorials: () => setTutorial('sources'), fullscreen: toggleFullscreen, escapeFullscreen: () => { if (!fullscreen) return false; if (fullscreenSearch) setFullscreenSearch(false); else toggleFullscreen(); return true },
    map, searchInput, sequence, selectedId, selected, query, view, expanded, blocked: dialog !== null || tutorial !== null, scopeKey,
    setQuery, setView, select: file => { selectFile(file); map.current?.focus(file); const index = search.results.findIndex(hit => hit.file.id === file.id); if (index >= 0) setActiveResult(index) },
    expand: expandPreview, close: () => { if (expanded) setExpanded(false); else hidePreview() }, showReader: showPreview, back,
    help: () => setDialog('commands'), metrics: () => setShowMetrics(value => !value), notice: setNotice,
    ...
    restore: place => { if (place.camera && place.cameraKey) writeStored('camera.continuous.' + place.cameraKey, place.camera); navigate(place.scope); setSelectedId(place.selectedId); if (place.camera && place.cameraKey === cameraKey) map.current?.restore(place.camera) },
  })

  return <div className={`atlas-shell atlas-theme-${theme} ${fullscreen ? 'atlas-fullscreen' : ''} ${fullscreenSearch ? 'atlas-fullscreen-search' : ''} ${expanded ? 'atlas-reading' : ''} ${sidebar ? '' : 'atlas-sidebar-hidden'} ${previewVisible && selected ? '' : 'atlas-preview-hidden'}`} style={{ '--reader-width': `${Math.max(300, Math.min(650, readerWidth))}px` } as CSSProperties}>
    <header className="atlas-topbar"><button className="atlas-brand" onClick={() => showOverview({})} aria-label="Star Palace home"><span aria-hidden="true">✳</span><strong>STAR PALACE</strong></button>
      <div className="atlas-search-field"><span aria-hidden="true">⌕</span><input ref={searchInput} onFocus={() => { if (fullscreen) setFullscreenSearch(true) }} value={query} placeholder="Find a file, a phrase, a place…" aria-label="Search library" onChange={e => setQuery(e.target.value)} onKeyDown={e => {
        if (e.key === 'Escape') { e.preventDefault(); e.currentTarget.blur(); setFullscreenSearch(false) }
        ...
        if (e.key === 'Enter') { e.preventDefault(); const hit = search.results[vim.searchDirection.current < 0 && activeResult === 0 ? search.results.length - 1 : activeResult]; if (hit) selectHit(hit); e.currentTarget.blur() }
      }} />{query ? <button aria-label="Clear search" onClick={() => setQuery('')}>×</button> : <kbd>⌘ / Ctrl K</kbd>}</div>
      <button className="atlas-top-action" onClick={() => setTutorial('sources')}>Tutorials</button><button className="atlas-top-action" onClick={() => setDialog('commands')}>Commands <kbd>:help</kbd></button><button className="atlas-icon-button" aria-label="Settings" onClick={() => setDialog('settings')}>⚙</button>
    </header>
    {fullscreen && <button className="atlas-fullscreen-toggle" aria-label="Exit fullscreen" aria-pressed={true} onClick={toggleFullscreen}>↙ Exit</button>}
    <div className="atlas-workspace">
      <nav id="atlas-library-navigation" className="atlas-sidebar" aria-label="Library navigation"><div className="atlas-sidebar-toolbar"><div className="atlas-eyebrow">Workspace</div><button className="atlas-icon-button" aria-label="Close library panel" onClick={closeLibrary}>×</button></div><button className={`atlas-nav-item ${!scope.regionId && !scope.collectionId ? 'is-active' : ''}`} onClick={() => showOverview({})}><span>✧ &nbsp; Your atlas</span><small>{summary.total.toLocaleString()}</small></button>
      ...
      {previewVisible && selected && <><div className="atlas-reader-resizer" role="separator" aria-label="Reader width" aria-orientation="vertical" tabIndex={0} onKeyDown={e => { if (e.key === 'ArrowLeft') setReaderWidth(w => Math.min(650, w + 20)); if (e.key === 'ArrowRight') setReaderWidth(w => Math.max(300, w - 20)) }} onPointerDown={e => {
      ...
      <Suspense fallback={<aside id="atlas-file-reader" className="atlas-reader"><p className="atlas-muted">Opening reader…</p></aside>}><Reader key={selected.id} file={selected} expanded={expanded} query={query} collections={collections} focusOnOpen={focusPreview} onExpand={expandPreview} onClose={hidePreview} onSelect={id => void selectId(id)} onChange={changeFile} hasSequence={sequence.length > 1} onPrevious={() => cycle(-1)} onNext={() => cycle(1)} /></Suspense></>}
    </div>
    ...
    {tutorial !== null && <Tutorials initial={tutorial} onClose={() => setTutorial(null)} />}
    <div className="atlas-sr-only" aria-live="polite">{query && search.status !== 'searching' ? `${search.results.length} results` : ''}</div>
  </div>
}

function CollectionDialog({ selection, activeId, selectedId, hits, query, busy, onClose, onAction, onDone }: { selection: string[] | null; activeId?: number; selectedId: string | null; hits: AtlasHit[]; query: string; busy: boolean; onClose: () => void; onAction: (op: () => Promise<void>) => Promise<void>; onDone: () => void }) {
