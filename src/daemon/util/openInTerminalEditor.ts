...
export function terminalEditorCommand(editorPath: string, filePath: string): string {
  ...
  return `exec ${quote(editorPath)} -- ${quote(filePath)}`
}
...
export async function openInTerminalEditor(file: Pick<FileNode, 'path' | 'mimeType' | 'platform'>, dependencies: TerminalEditorDependencies = {}): Promise<{ editor: TerminalEditorName }> {
  ...
  try {
    ...
    if (!(await handle.stat()).isFile()) throw new TerminalEditorError('Choose a regular text file to edit.', 400)
  ...
  const command = terminalEditorCommand(editor.path, file.path)
